...
- The "five minutes of grep" calibration is for the pack's typical repo sizes; on very large codebases impact analysis is genuinely harder and tooling-dependent (LSP, code search), and the skill's escalation valve (growing map → S2 → yo-design) is the honest answer rather than pretending grep scales forever.
